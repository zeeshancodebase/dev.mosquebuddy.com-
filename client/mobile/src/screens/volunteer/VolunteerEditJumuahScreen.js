// src/screens/volunteer/VolunteerEditJumuahScreen.js
...
export default function VolunteerEditJumuahScreen({ navigation, route }) {
  const venueParam = route.params?.venue;
  const venueId = venueParam?.id;
  const permissions = route.params?.permissions || {};
  const canEdit = !!permissions.canUpdateTimings;
  ...
  async function handleSave(values) {
    ...
    try {
      ...
      if (editingSlot?.id) {
        await updateVolunteerJumuahTiming(editingSlot.id, payload);
      } else {
        await createVolunteerJumuahTiming(venueId, payload);
      }
        ...
        >
          {slots.length === 0 ? (
            <EmptyState
              ...
              subtitle="Add this mosque's Friday prayer slot so nearby Muslims can find it."
            />
          ) : (
            slots.map((slot) => (
...
const styles = StyleSheet.create({
  ...
  venueBadges: { alignItems: "flex-start", flexDirection: "row", gap: 8 },
  languageBadge: { fontSize: 12, color: "#92400E", backgroundColor: "#FEF3C7", fontWeight: "600", paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  womenBadge: { backgroundColor: "#FEF3C7", paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  womenBadgeText: { fontSize: 12, color: "#92400E", fontWeight: "600" },
});