// src/screens/mosqueAdmin/MosqueAdminEditTimingsScreen.js
...
import { updateMyDailyTiming, createMyDailyTiming } from "../../lib/endpoints";
...
export default function MosqueAdminEditTimingsScreen({ navigation, route }) {
  const { venue, permissions = {} } = route.params || {};
  const canMarkVerified = !!permissions.canMarkVerified;
  ...
  const [activePicker, setActivePicker] = useState(null); // { prayer, field }
  const [saving, setSaving] = useState(false);

  const [effectiveMode, setEffectiveMode] = useState("today"); // 'today' | 'custom'
  ...
  const dateValid =
    effectiveMode === "today" || !!customDate;
  ...
  async function handleSave() {
    ...
    const verificationStatus = canMarkVerified ? "verified" : "community_updated";
    ...
    for (const prayer of dirtyPrayers) {
      ...
      try {
        ...
        if (t.id) {
          await updateMyDailyTiming(t.id, body);
        } else {
          const res = await createMyDailyTiming(venue.id, body);
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.tableHeader}>
          ...
          <View style={{ width: 12 }} />
          {/* <Text style={styles.tableHeaderText}>Time Type</Text> */}
          <TouchableOpacity
          ...
          {effectiveMode === "custom" && (
            <>
              ...
              >
                <Text
                  style={{
                    color: customDate ? COLORS.textPrimary : COLORS.textMuted,
                    fontSize: 14,
                  }}
                >
...
// import IslamicPattern from "../../components/IslamicPattern";
// import TimePicker from "../../components/TimePicker";
// import { updateMyDailyTiming, createMyDailyTiming, updateMyJumuahTiming, createMyJumuahTiming } from "../../lib/endpoints";
// import { formatTime12h } from "../../lib/dateUtils";
...
// // Builds initial state from venue's existing timings
// function buildTimingState(dailyTimings = []) {
//   const state = {};
//   PRAYER_ORDER.forEach((prayer) => {
//     const existing = dailyTimings.find((t) => t.prayerName === prayer);
...
// function PrayerEditRow({ prayer, data, onEditJamaah, onEditAzaan, saving }) {
//   const hasJamaah = !!data.jamaahTime;
//   const isRelative = data.timingType === "relative";
...
//       <TouchableOpacity
//         style={[styles.timeCell, hasJamaah && styles.timeCellActive, data.dirty && styles.timeCellDirty]}
//         onPress={() => onEditJamaah(prayer)}
//         activeOpacity={0.75}
//         disabled={saving}
//       >
//         <Text style={styles.timeCellLabel}>Jamā'ah</Text>
//         <Text style={[styles.timeCellValue, !hasJamaah && styles.timeCellEmpty, hasJamaah && styles.timeCellValueActive]}>
//           {isRelative ? data.relativeTimeText : (formatTime12h(data.jamaahTime) || "Tap to set")}
//         </Text>
//       </TouchableOpacity>

//       {data.dirty && <Text style={styles.dirtyDot}>●</Text>}
//     </View>
//   );
// }
...
//   const dirtyPrayers = Object.keys(timings).filter((p) => timings[p].dirty);
//   const hasDirty = dirtyPrayers.length > 0;
...
//           prayerName: prayer,
//           timingType: t.timingType,
//           ...(t.jamaahTime && { jamaahTime: t.jamaahTime }),
//           ...(t.azaanTime && { azaanTime: t.azaanTime }),
//           ...(t.relativeTimeText && { relativeTimeText: t.relativeTimeText }),
...
//         } else {
//           const res = await createMyDailyTiming(venue.id, body);
//           // Store new id so subsequent saves are updates not creates
//           setTimings((prev) => ({
//             ...prev,
//             [prayer]: { ...prev[prayer], id: res.data?.id, dirty: false },
//           }));
//         }
//         successCount++;
//         setTimings((prev) => ({
//           ...prev,
//           [prayer]: { ...prev[prayer], dirty: false },
//         }));
//       } catch (e) {
//         errors.push(`${PRAYER_NAMES[prayer]}: ${e.message}`);
//       }
...
//       Alert.alert(
//         "Timings Updated ✓",
//         `${successCount} timing${successCount > 1 ? "s" : ""} saved and marked as Verified.`,
//         [{ text: "Done", onPress: () => navigation.goBack() }]
//       );
//     }
//   }
...
//             Tap any time to update it. Changes are saved as <Text style={{ fontWeight: "700", color: COLORS.primary }}>Verified</Text> — only update times you've personally confirmed.
//           </Text>
//         </View>

//         <View style={styles.tableHeader}>
//           <Text style={[styles.tableHeaderText, { flex: 1.2 }]}>PRAYER</Text>
//           <Text style={styles.tableHeaderText}>AZAAN</Text>
//           <Text style={styles.tableHeaderText}>JAMĀ'AH</Text>
//           <View style={{ width: 16 }} />
//         </View>
...
//             prayer={prayer}
//             data={timings[prayer]}
//             onEditJamaah={(p) => setActivePicker({ prayer: p, field: "jamaah" })}
//             onEditAzaan={(p) => setActivePicker({ prayer: p, field: "azaan" })}
//             saving={saving}
//           />
//         ))}
...
//   dirtyDot: { fontSize: 10, color: "#D97706", width: 16, textAlign: "center" },
